...
from mlstorlets.sgdproxies.SGDRegressorProxy  import SGDRegressorProxy
from sklearn.datasets.samples_generator import make_blobs
import numpy as np


class TestSDGRegressorProxy(unittest.TestCase):
    ...
    def _fit_iter(self, est, X, Y):
        ...
        for X_chunk, y_chunk in batcherator:
            if isinstance(est, SGDRegressor):
    ...
    def test_basic_func(self):
        ...
        Xtest = [[0.5, 0.7], [1, 2]]
        model = SGDRegressor(n_iter=10000, epsilon=0.0001, shuffle=False)
        self._fit_iter(model, X, Y)
        p1 = self._predict(model, Xtest)

        model = SGDRegressorProxy(n_iter=10000, epsilon=0.0001, shuffle=False)
        self._fit_iter(model, X, Y)
        print "QQQ"
